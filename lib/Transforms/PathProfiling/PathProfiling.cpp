...
namespace llvm {
  ...
  public:
    static StructType *get(LLVMContext& C) {
		...
		std::vector<Type*> elementTypes(3);
		elementTypes[0] = TypeBuilder<types::i<32>, xcompile>::get(C); // type
		elementTypes[1] = TypeBuilder<types::i<32>, xcompile>::get(C); // array size
		elementTypes[2] = TypeBuilder<types::i<8>*, xcompile>::get(C); // array/ hash pointer
		llvm::ArrayRef<llvm::Type *> arElementTypes(elementTypes);


    	return( StructType::get(C, arElementTypes, NULL));
    }
	};
...
void BLInstrumentationDag::generateDotGraph() {
  ...
  std::string functionName = getFunction().getName();
...
void BLInstrumentationNode::setStartingPathNumber(Value* pathNumber) {
	DEBUG(dbgs() << "  SPN-" << getName() << " <-- " << (pathNumber ? pathNumber->getName() : "unused") << "\n");
...
void BLInstrumentationNode::setEndingPathNumber(Value* pathNumber) {
	DEBUG(dbgs() << "  EPN-" << getName() << " <-- " << (pathNumber ? pathNumber->getName() : "unused") << "\n");
...
void PathProfiler::preparePHI(BLInstrumentationNode* node) {
	...
	PHINode* phi = PHINode::Create(Type::getInt32Ty(*Context), 0, "pathNumber",
		insertPoint );
	node->setPathPHI(phi);
...
void PathProfiler::insertCounterIncrement(Value* incValue,
    ...
    llvm::ArrayRef<llvm::Value *> arGepIndices(gepIndices);

    GetElementPtrInst* pcPointer =
      GetElementPtrInst::Create(dag->getCounterArray(),
    		  arGepIndices, "counterInc", insertPoint);
